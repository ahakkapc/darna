...
import { CheckCircle, ShoppingCart, Users, ShieldCheck, Gear, ChatDots, Trash } from '@phosphor-icons/react';
...
const CATEGORIES = [
  ...
  { value: 'LISTING', label: 'Annonces' },
  { value: 'INBOX', label: 'Messagerie' },
...
export default function NotificationCenterPage() {
  ...
  const markAllRead = async () => {
    try {
      await http.post('/notifications/read-all', category ? { category } : {});
  ...
  const actions = (
    <Button size="small" onClick={markAllRead} sx={{ color: 'var(--brand-copper)', fontSize: '13px' }}>
      Tout marquer comme lu
    </Button>
  );

  return (
    <DPage title="Centre de notifications" subtitle="Toutes vos notifications" actions={actions}>
      <Box sx={{ display: 'flex', gap: 'var(--space-12)', mb: 'var(--space-16)', flexWrap: 'wrap', alignItems: 'center' }}>
        <ToggleButtonGroup
          size="small"
          exclusive
          value={filter}
        ...
        <Select
          ...
          displayEmpty
          sx={{ fontSize: '12px', minWidth: 120, '& .MuiSelect-select': { py: '6px' } }}
        >
          {CATEGORIES.map((c) => (
                    ...
                    {!n.readAt && (
                      <Box
                        sx={{
                          ...
                          backgroundColor: 'var(--brand-copper)', flexShrink: 0,
                        }}
                      />
                    )}
                    <Typography
                      sx={{
                        fontWeight: n.readAt ? 400 : 600, fontSize: '13px',
